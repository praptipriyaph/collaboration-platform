...
The system runs as a distributed cluster of 5 nodes:

1 LLM Server: A standalone gRPC server that runs a local LLM (e.g., TinyLlama) to handle AI queries.

3 App Server Nodes (Raft Cluster): A cluster of three servers that form a Raft consensus group. They manage the replicated log, elect a leader, and maintain the document state.

1 Client Node(s): A "smart" CLI client that connects to the cluster, automatically finds the leader, and subscribes to real-time event streams.

Features Implemented

Raft Consensus: Full from-scratch implementation of leader election, log replication, and failure detection.

Fault Tolerance: The cluster survives the loss of any single node (including the leader) and will elect a new leader.

Real-time Updates: Uses gRPC server-side streaming (Subscribe) to instantly push document changes (CREATE, UPDATE) to all connected clients.

Conflict Resolution: A simplified distributed locking mechanism (LOCK, UNLOCK) managed via Raft. Updates are rejected if the user does not hold the lock.

Real-time Presence: JOIN and LEAVE events are broadcast to all clients as users connect and disconnect from the real-time stream.

Local LLM Integration: An independent gRPC server (llm_server) loads a GGUF model (e.g., TinyLlama) using ctransformers to provide real grammar, summarization, and content enhancement.

Smart Client: The CLI client (client.py) automatically retries on failure, handles leader redirection, and listens for real-time alerts in a background thread.

How to Run the System

1. Setup

Prerequisites: Python 3.10+, pip, and wget (or you can download manually).
