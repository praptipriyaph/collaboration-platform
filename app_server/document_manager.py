...
class DocumentManager:
    ...
    def update_document(self, doc_id, content, username):
        if doc_id not in self.documents:
            print(f"DEBUG: Update failed. Document {doc_id} not found.")
            return False

        # Conflict Resolution Check
        if doc_id in self.locks and self.locks[doc_id] != username:
            print(f"DEBUG: Update failed. Doc is locked by '{self.locks[doc_id]}', but '{username}' tried to update it.")
            return False
    ...
    def acquire_lock(self, doc_id, username):
        if doc_id not in self.documents:
            print(f"DEBUG: Lock failed. Document {doc_id} not found.")
            return False
        if doc_id not in self.locks or self.locks[doc_id] == username:
            ...
            return True
        print(f"DEBUG: Lock failed. Already locked by {self.locks[doc_id]}.")
        return False
    ...
    def release_lock(self, doc_id, username):
        if doc_id in self.locks and self.locks[doc_id] == username:
            ...
            return True
        print(f"DEBUG: Unlock failed. Doc is locked by {self.locks.get(doc_id, 'NO ONE')}, requester was {username}.")
        return False
